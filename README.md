...
小程序与 FastAPI 后端共用仓库根目录。`project.config.json` 已通过 `packOptions.ignore`
排除 `backend`、后台头像素材、数据库文件和项目文档，并关闭上传 Source Map；这些文件仍保留在
仓库中，但不会进入微信小程序代码包。修改打包配置时不要移除这些排除项。
